...
void uart_init(uint32_t clock_speed, uint32_t baud_rate)
{
	...
	// This is very size sensitive. If allocation fails, it can have obscure consequences. We think allocation failure is due to malloc() failing
	cb_allocate(&tx_buffer, 128, sizeof(unsigned char));
	cb_allocate(&rx_buffer, 128, sizeof(unsigned char));
